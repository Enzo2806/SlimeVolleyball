...
class DDQN_Agent(object):
	...
	def train(self, replay_buffer):
		s, a, r, s_next, dw, ind, Normed_IS_weight = replay_buffer.sample(self.batch_size)
		...
		# BP
		q_loss = torch.square((~False) * Normed_IS_weight * (Q_target - current_Q)).mean()  #这里好像不用乘(~tr)
	...
	def save(self, folder_path, agent_number, n):
		"""
        Save the models
        """
		torch.save(self.q_net.state_dict(), f"{folder_path}/step-{n}-agent-{agent_number}-q_net.pt")

	def load(self, folder_path, agent_number, n):
		"""
        Load the models
        """
		self.q_net.load_state_dict(torch.load(f"{folder_path}/step-{n}-agent-{agent_number}-q_net.pt"))
		self.q_target.load_state_dict(torch.load(f"{folder_path}/step-{n}-agent-{agent_number}-q_net.pt"))
